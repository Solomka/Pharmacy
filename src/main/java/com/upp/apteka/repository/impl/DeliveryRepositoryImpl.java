...
public class DeliveryRepositoryImpl extends AHibernateRepository<Delivery, Long> implements DeliveryRepository {
	...
	private final String medicineNameRestriction = "({alias}.id IN " + "(SELECT id_delivery" + " FROM delivery_medicine"
			+ " WHERE id_medicine IN (SELECT id" + " FROM medicine" + " WHERE UPPER(name) LIKE UPPER('%?%'))))";
	private final String medicineProducerRestriction = "({alias}.id IN " + "(SELECT id_delivery"
			+ " FROM delivery_medicine" + " WHERE id_medicine IN (SELECT id " + " FROM medicine"
			+ " WHERE UPPER(producer) LIKE UPPER('%?%'))))";

	@SuppressWarnings("unchecked")
	public List<Delivery> getAll(int offset, int limit) {

		return (List<Delivery>) createEntityCriteria().setFirstResult(offset).setMaxResults(limit)
	...
	public List<Delivery> findPharmacyDeliveriesByPeriod(Date from, Date to, Long pharmacyId, int offset, int limit) {
		...
		return (List<Delivery>) criteria.setFirstResult(offset).setMaxResults(limit).addOrder(Order.desc("date"))
				.list();
	...
	/*
	 * check if at least one medicine was sold after this medicine's the most
	 * recent delivery
	 */
	public boolean checkIfDeliveryMedicineSold(Long deliveryId) {

		String hql = " SELECT p" + " FROM Purchase AS p" + " WHERE p.date >= (SELECT d.date" + " FROM Delivery AS d"
				+ " WHERE d.id = :deliveryId)" + " AND p.id IN (SELECT p1.purchMedicine.purchase.id"
				+ " FROM p.purchaseMedicines AS p1" + " WHERE p1.purchMedicine.medicine.id IN "
				+ " (SELECT dm.deliveryMedicineID.medicine.id" + " FROM DeliveryMedicine AS dm"
	...
	public List<DeliveryMedicine> getDeliveryMedicines(Long deliveryId, int offset, int limit) {
		...
		String hql = "SELECT dM" + " FROM DeliveryMedicine AS dM"
	...
	public int count(String query, Date start, Date finish, boolean or) {
		...
		return ((Number) prepareQueryStatement(query, start, finish, or).setProjection(Projections.rowCount())
	...
	private Criteria prepareQueryStatement(String query, Date start, Date finish, boolean or) {
		...
		//get deliveries of the particular pharmacy
		conjunction.add(Restrictions.eq("pharmacy.id", pharmacy.getId()));
		/*
		 * add date if exists
		 */
